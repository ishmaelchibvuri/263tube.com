...
const TABLE_NAME = process.env.TABLE_NAME || "exam-platform-data";
...
async function getUserByEmail(email: string): Promise<any | null> {
  ...
  const user = unmarshall(firstItem);
 ...
 */
async function getUserItems(userId: string): Promise<DynamoItem[]> {
 ...
 */
async function deleteItemsBatch(items: DynamoItem[]): Promise<void> {
